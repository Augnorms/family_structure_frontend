...
export const FilesUploads = (props:FileInterface) => {
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    if (files) {
        ...
        const isValidSize = fileListArray.every(file => file.size <= 5 * 1024 * 1024); // 5 MB in bytes
            ...
            const isValidFiles = fileListArray.every(file => {
                ...
                return file.size <= 5 * 1024 * 1024 && (isImage || isPdf || isWord || isExcel);
              });
  ...
  return (
    ...
    onDrop={handleDrop}
  >
    {props.fileData.length === 0 ? (<div className='w-full' >
     ...
     (
        <div className="w-full">
          {props.fileData.map((data, idx) => (
             <div key={idx} className='flex justify-center'>
               ...
               <CiTrash 
                 style={{ marginTop: '5px', marginLeft:'5px', cursor:'pointer', color:'red'}} 
       ...
       {maxFile ? <div className='mt-2 text-red-500'>File exceeds 5mb</div> : <div></div>} 
  </div>
  )
}
